    ...
    Pipe p)
{
    ...
    status_t ret = serviceMgr->addService(generateServiceName(num), service);
    if (ret != OK)
      exit(EXIT_FAILURE);

    ALOGE("Registered service with service manager");
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        ret = workers[target]->transact(BINDER_NOP, data, &reply);
        if (ret != OK)
          exit(EXIT_FAILURE);
