    ...
    public:
        ...
        BinderLibTestBundle(const Parcel *source) : m_isValid(false) {
            ...
            if (source->readInt32(&mark)) {
                ALOGE("Cannot read mark");
                return;
            }
            if (mark != MARK_START) {
                ALOGE("Mark is not started");
                return;
            }
            if (source->readInt32(&bundleLen)) {
                ALOGE("Cannot read bundle length");
                return;
            }
            pos = source->dataPosition();
            if (Parcel::appendFrom(source, pos, bundleLen)) {
                ALOGE("Cannot append to parcel");
                return;
            }
            source->setDataPosition(pos + bundleLen);
            if (source->readInt32(&mark)) {
                ALOGE("Cannot read mark 2");
                return;
            }
            if (mark != MARK_END) {
                ALOGE("Mark is not ended");
                return;
            }
            m_isValid = true;
            setDataPosition(0);
...
TEST_F(BinderLibTest, DISABLED_IndirectGetId3)
{
    status_t ret;
    int32_t id;
    int32_t count;
...
TEST_F(BinderLibTest, DISABLED_DeathNotificationNoRefs)
{
    status_t ret;
...
TEST_F(BinderLibTest, DISABLED_DeathNotificationWeakRef)
{
    status_t ret;
    wp<IBinder> wbinder;
...
TEST_F(BinderLibTest, DISABLED_DeathNotificationStrongRef)
{
    status_t ret;
    sp<IBinder> sbinder;
...
TEST_F(BinderLibTest, DISABLED_DeathNotificationMultiple)
{
    status_t ret;
...
TEST_F(BinderLibTest, DISABLED_PassFile) {
    int ret;
    int pipefd[2];
    uint8_t buf[1] = { 0 };
...
TEST_F(BinderLibTest, DISABLED_PromoteRemote) {
    int ret;
    Parcel data, reply;
    sp<IBinder> strong = new BBinder();
    sp<IBinder> server = addServer();
    ...
    public:
        ...
        virtual status_t onTransact(uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags = 0) {
            ...
            case BINDER_LIB_TEST_EXIT_TRANSACTION:
                // HACK: We need a way to terminate all our spawned child processes
                // and we simply send SIGTERM to all processes in our process group.
                // On some shells this might print a message but that is accepted.
                kill(0, SIGTERM);
