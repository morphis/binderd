...
class BpDemo : public BpInterface<IDemo> {
    public:
        ...
        virtual int32_t add(int32_t v1, int32_t v2) {
            ...
            ALOGE("BpDemo::add(%i, %i) = %i (status: %i)", v1, v2, res, status);
            return res;
        }
};
...
status_t BnDemo::onTransact(uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags) {
    ...
    switch(code) {
        ...
        case PUSH: {
            ...
            ALOGE("BnDemo::onTransact got %i", inData);
            push(inData);
            ASSERT(reply != 0);
            reply->print(PLOG); endl(PLOG);
            return NO_ERROR;
        ...
        case ADD: {
            ...
            ALOGE("BnDemo::onTransact add(%i, %i) = %i", inV1, inV2, sum);
            ASSERT(reply != 0);
            reply->print(PLOG); endl(PLOG);
...
int main(int argc, char **argv) {
    if (argc == 1) {
        ...
        ALOGE("Addition result: %i + %i = %i", v, adder, sum);
    }
